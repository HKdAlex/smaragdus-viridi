...
interface CartContextType {
  ...
  // Actions
  addToCart: (gemstoneId: string) => Promise<boolean>;
  ...
  isInCart: (gemstoneId: string) => boolean;
  getCartItemQuantity: (gemstoneId: string) => number;
...
export function CartProvider({ children, userId }: CartProviderProps) {
  ...
  const addToCart = useCallback(
    async (gemstoneId: string): Promise<boolean> => {
      ...
      if (!userId) {
        ...
        console.error("❌ CartContext.addToCart:", errorMsg);
        setError(errorMsg);
        return false;
      }
      ...
      try {
        ...
        if (result.success && result.cart_summary) {
          console.log("✅ Successfully added to cart, updating context state");
  ...
  const isInCart = useCallback(
    (gemstoneId: string): boolean => {
      return (
        cartSummary?.items.some((item) => item.gemstone_id === gemstoneId) ||
        false
      );
    },
    [cartSummary]
  ...
  const getCartItemQuantity = useCallback(
    (gemstoneId: string): number => {
      const item = cartSummary?.items.find(
        (item) => item.gemstone_id === gemstoneId
      );
  ...
  const toggleSelectAll = useCallback(() => {
    if (cartSummary) {
      const allSelected = cartSummary.items.every((item) =>
