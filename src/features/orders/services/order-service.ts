...
export class OrderService {
  ...
  async createOrder(request: CreateOrderRequest): Promise<OrderResult> {
    try {
      this.logger.info('Creating order', { userId: request.user_id, itemCount: request.items.length })
      ...
      const { data: order, error: orderError } = await supabase
        ...
        .insert({
          ...
          currency_code: request.currency_code as 'USD' | 'EUR' | 'GBP' | 'RUB' | 'CHF' | 'JPY',
      ...
      const paymentReference = `PAY-${order.id.slice(0, 8).toUpperCase()}-${Date.now()}`
      ...
      return {
        ...
        order: {
          ...
          currency_code: order.currency_code as 'USD' | 'EUR' | 'GBP' | 'RUB' | 'CHF' | 'JPY',
        ...
        order: {
          ...
          items: order.order_items.map((item: any) => ({
            ...
            gemstone: item.gemstones ? {
              ...
              images: item.gemstones.gemstone_images?.map((img: any) => ({
                id: img.id,
