#!/usr/bin/env node
/**
 * Test batch: Process 50 gemstones with 10 concurrent workers
 */
...
// ============================================================================
// FETCH 50 GEMSTONES TO TEST
// ============================================================================

async function getTestGemstones() {
  // Get gemstones WITH images for full AI capability testing
  // Query gemstones that have at least 3 images
  const { data: gemsWithImages, error: imgError } = await supabase
  ...
  const imageCounts = gemsWithImages.reduce((acc, { gemstone_id }) => {
  ...
  console.log(
    `   Found ${gemstoneIdsWithImages.length} gemstones with 3+ images`
  );
...
async function main() {
  console.log("=".repeat(80));
  console.log("AI V6 TEST BATCH - 50 GEMSTONES WITH 10 WORKERS");
  console.log("=".repeat(80));

  console.log("\n🔍 Fetching 50 gemstones to test...");
  ...
  if (successRate >= 95 && stats.successCount >= 40) {
