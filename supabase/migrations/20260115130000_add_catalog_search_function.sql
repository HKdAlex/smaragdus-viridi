-- Migration: Add catalog_search_gemstones function for proper search handling
-- Fixes PostgREST ::text casting issue in catalog API search queries
-- 
-- This function handles search with proper enum-to-text casting that PostgREST doesn't support
...
BEGIN
  ...
  WHERE 
    ...
    -- Search condition (with proper ::text casting)
    AND (
      search_pattern IS NULL 
      OR g.serial_number ILIKE search_pattern
      OR g.name::text ILIKE search_pattern
      OR g.color::text ILIKE search_pattern
      OR COALESCE(cu.code, g.cut_code)::text ILIKE search_pattern
    )
    ...
    -- Color filter
    AND (filter_colors IS NULL OR cardinality(filter_colors) = 0 OR g.color::text = ANY(filter_colors))
    
    -- Cut filter
    AND (filter_cuts IS NULL OR cardinality(filter_cuts) = 0 OR COALESCE(cu.code, g.cut_code) = ANY(filter_cuts))
    
    -- Clarity filter
    AND (filter_clarities IS NULL OR cardinality(filter_clarities) = 0 OR g.clarity::text = ANY(filter_clarities))
  ...
  ORDER BY 
    CASE WHEN sort_direction = 'asc' THEN
      CASE sort_by
        WHEN 'price_amount' THEN g.price_amount
        WHEN 'weight_carats' THEN g.weight_carats
        WHEN 'name' THEN g.name::text
        WHEN 'color' THEN g.color::text
        WHEN 'cut' THEN COALESCE(cu.code, g.cut_code)::text
        ELSE g.created_at
      END
    END ASC,
...
COMMENT ON FUNCTION catalog_search_gemstones IS 'Catalog search function with proper enum-to-text casting for PostgREST compatibility';
