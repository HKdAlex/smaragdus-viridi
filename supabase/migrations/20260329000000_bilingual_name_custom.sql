-- Bilingual custom display names: name_custom_en / name_custom_ru
-- Keeps legacy name_custom; backfills EN/RU from it where null.

ALTER TABLE public.gemstones
  ADD COLUMN IF NOT EXISTS name_custom_en text,
  ADD COLUMN IF NOT EXISTS name_custom_ru text;

COMMENT ON COLUMN public.gemstones.name_custom_en IS 'Admin display name for EN storefront';
COMMENT ON COLUMN public.gemstones.name_custom_ru IS 'Admin display name for RU storefront';

UPDATE public.gemstones
SET
  name_custom_en = COALESCE(NULLIF(TRIM(name_custom_en), ''), NULLIF(TRIM(name_custom), '')),
  name_custom_ru = COALESCE(NULLIF(TRIM(name_custom_ru), ''), NULLIF(TRIM(name_custom), ''))
WHERE NULLIF(TRIM(name_custom), '') IS NOT NULL;

-- Enriched view: expose bilingual columns (display_name unchanged for legacy SQL)
...
-- catalog_search_gemstones: return bilingual name columns + type_code for storefront
...
COMMENT ON FUNCTION public.catalog_search_gemstones IS
  'Catalog search with primary image ordering; includes name_custom_en/name_custom_ru/type_code for bilingual titles.';

-- PG cannot change RETURNS TABLE shape with CREATE OR REPLACE; drop first.
DROP FUNCTION IF EXISTS public.search_gemstones_multilingual(text, integer, integer, text, boolean, jsonb);

-- search_gemstones_multilingual: expose bilingual custom names on result rows
...
BEGIN
  ...
  SELECT
    ...
    counted.cut_id,
    counted.name_custom,
    counted.name_custom_en,
    counted.name_custom_ru,
    counted.created_at,
    counted.updated_at,
    counted.total_count,
    counted.rank
...
COMMENT ON FUNCTION search_gemstones_multilingual IS
  'Full-text search with serial/internal_code matching; returns name_custom_en/name_custom_ru for bilingual titles.';
