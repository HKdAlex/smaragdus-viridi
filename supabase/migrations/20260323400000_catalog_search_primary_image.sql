-- Align catalog list thumbnail URL with detail page / selectPrimaryImage:
-- 1) AI selected_image_uuid (when that row exists)
-- 2) is_primary
-- 3) recommended_primary_image_index (0-based, by image_order)
-- 4) first image by image_order
-- Plus COALESCE(gemstones.primary_image_url) when no gemstone_images rows exist.
--
-- Previously the RPC used only ORDER BY image_order ASC LIMIT 1, so the grid
-- often showed the wrong image when the primary was not the lowest image_order.
...
COMMENT ON FUNCTION public.catalog_search_gemstones IS
  'Catalog search with space-normalized serial/internal code matching. List primary_image_url matches detail hero order: selected_image_uuid, is_primary, recommended index, then image_order.';
