-- Add internal_code to catalog_search_gemstones return type and SELECT.
-- This allows the catalog card to display the human-readable internal code
-- (e.g. "И 447") instead of the system serial number (e.g. "SV-447-...").
-- DROP required because PostgreSQL cannot change the return type of an existing function.
...
COMMENT ON FUNCTION public.catalog_search_gemstones IS
  'Catalog search with space-normalized serial/internal code matching (V17 matches V 17). Display fields with Admin Custom > AI > Enum precedence.';
